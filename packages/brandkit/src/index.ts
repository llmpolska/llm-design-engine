...
export function generateBrandkit(
  ...
  const imagePrompts: AssetRequirement[] = [
    {
      id: `${project}-hero-art`,
      role: 'hero artwork',
      prompt: `Tactile editorial material study for ${direction.metaphor}; ${direction.materialLanguage.join(', ')}; one clear focal object; photographed like a working artifact`,
      negativeConstraints: [
        'stock photo',
        'generic SaaS UI',
        'purple gradient',
      ...
      aspectRatio: '16:10',
      intendedPlacement: 'landing hero',
      generatedFile: 'assets/hero-art.svg',
      provider: 'mock',
      model: 'deterministic-svg',
      generatedAt,
    },
    {
      id: `${project}-blueprint-art`,
      role: 'meaning to blueprint illustration',
      prompt: `A sequence of annotated marks compressing ${direction.visualNarrative} into an executable blueprint; paper, plotter ink, registration brackets`,
      negativeConstraints: ['abstract blob', 'neon circuit board', 'unrelated stock imagery'],
      aspectRatio: '4:3',
      intendedPlacement: 'process section',
      generatedFile: 'assets/blueprint-art.svg',
      provider: 'mock',
      model: 'deterministic-svg',
      generatedAt,
    },
    {
      id: `${project}-material-study`,
      role: 'material study',
      prompt: `Close crop of ${direction.materialLanguage.join(', ')} arranged as domain evidence for ${direction.name}; high contrast, honest texture`,
      negativeConstraints: ['3D render', 'generic device mockup', 'glow'],
      aspectRatio: '3:2',
      intendedPlacement: 'direction detail',
      generatedFile: 'assets/material-study.svg',
      provider: 'mock',
      model: 'deterministic-svg',
      generatedAt,
    },
    {
      id: `${project}-social-preview`,
      role: 'social preview',
      prompt: `Editorial title plate for ${projectId} using the ${direction.metaphor} visual language; readable mark, paper and ink, no UI screenshot`,
      negativeConstraints: ['stock image', 'generic AI imagery', 'purple cloud'],
      aspectRatio: '1.91:1',
      intendedPlacement: 'Open Graph image',
      generatedFile: 'assets/social-preview.svg',
      provider: 'mock',
      model: 'deterministic-svg',
      generatedAt,
    },
  ...
  return {
    ...
    typography: [
      {
        role: 'display',
        family: 'Arial Narrow / Trebuchet MS',
        usage: 'short high-contrast headlines and route names',
      },
      {
        role: 'body',
        family: 'Trebuchet MS / system sans-serif',
        usage: 'explanations, labels, and instructions',
      },
      {
        role: 'mono',
        family: 'ui-monospace',
        usage: 'metadata, measurements, command output, annotations',
      },
    ],
    spacingPrinciples: [
    ...
    socialPreviewConcept:
      'A paper title plate with a single route line crossing the mark and project name.',
    imagePrompts,
    usageRules: [
      'Keep one accent color dominant.',
      'Pair every visual decision with a domain reason.',
      'Use the mark as a registration device, not a decoration.',
      'Let text and material share the same compositional route.',
    ],
    misuseRules: [
      'Do not add purple gradients or neon glows.',
      'Do not turn every section into a rounded card.',
      'Do not center every heading by default.',
      'Do not use stock photos, generic dashboard mockups, or abstract blobs.',
      'Do not recreate the mark as a sparkle, wand, robot, brain, or infinity loop.',
    ],
    generatedAt,
  };
}
...
export function brandkitTokenCss(brand: Brandkit): string {
  const colorTokens = brand.colors
    .map((color) => `--brand-${slugify(color.name)}:${color.hex};`)
    .join('');
  const typographyTokens = brand.typography
    .map((font) => `--brand-font-${slugify(font.role)}:${font.family};`)
    .join('');
...
export function brandkitAssetManifest(brand: Brandkit): string {
  return JSON.stringify(
    { projectId: brand.projectId, generatedAt: brand.generatedAt, assets: brand.imagePrompts },
    null,
    2,
  );
}
