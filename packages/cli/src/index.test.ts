...
describe('lde CLI workflow', () => {
  it('initializes a project and writes a brief without credentials', async () => {
    ...
    await runCommand(['brief', '--name', 'GastroOps', '--summary', 'Restaurant operations', '--domain', 'restaurant operations', '--tension', 'Keep service legible under pressure'], { cwd, output });
  ...
  it('runs directions, generate, lint, brandkit and export locally', async () => {
    ...
    const output: string[] = [];
    await runCommand(['init'], { cwd, output });
    await runCommand(['brief', '--name', 'GastroOps', '--summary', 'Restaurant operations', '--domain', 'restaurant operations'], { cwd, output });
    ...
    expect(await readFile(join(cwd, '.design', 'pages', 'landing.design.md'), 'utf8')).toContain('# Narrative');
    expect(await readFile(join(cwd, '.design', 'BRAND.md'), 'utf8')).toContain('Usage and misuse');
    expect(await readFile(join(cwd, '.design', 'EXPORT.md'), 'utf8')).toContain('Agent implementation instructions');
  });
});
