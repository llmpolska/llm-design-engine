...
import type { BriefInput, Brandkit, CreativeDirection, DesignDocument, ProjectBrief } from '@llm-design-engine/core';
import { createProjectBrief } from '@llm-design-engine/core';
import { lintDesign, formatLintReport } from '@llm-design-engine/anti-slop';
import { generateBrandkit, generatePressMarkSvg, brandkitTokenCss } from '@llm-design-engine/brandkit';
import { MockReasoningProvider, OpenAIReasoningProvider, type ReasoningProvider } from '@llm-design-engine/creative-director';
...
async function ensureDesignWorkspace(cwd: string): Promise<void> {
  ...
  await writeFile(join(root, 'config.json'), JSON.stringify({ version: 1, provider: 'mock', preview: { port: 4317 } }, null, 2));
  await writeFile(join(root, 'manifest.json'), JSON.stringify({ version: 1, projectId: null, artifacts: ['BRIEF.md', 'DIRECTIONS.md', 'BRAND.md', 'pages/landing.design.md', 'brandkit.json', 'previews/landing.html', 'EXPORT.md'] }, null, 2));
}
...
function briefMarkdown(brief: ProjectBrief): string {
  return ['---', `id: ${brief.id}`, `name: ${brief.name}`, `domain: ${brief.domain}`, '---', '# Product brief', brief.summary, '', '## Users', ...brief.users.map((item) => `- ${item}`), '', '## Jobs', ...brief.jobs.map((item) => `- ${item}`), '', '## Tension', brief.tension || 'Not specified yet.', '', '## Constraints', ...(brief.constraints.length > 0 ? brief.constraints.map((item) => `- ${item}`) : ['- None recorded.']), '', '<!-- lde-brief-data', JSON.stringify(brief, null, 2), '-->', ''].join('\n');
}
...
function directionsMarkdown(brief: ProjectBrief, interpretation: unknown, directions: CreativeDirection[]): string {
  const rows = directions.map((direction, index) => [`## ${index + 1}. ${direction.name}`, `- id: ${direction.id}`, `- metaphor: ${direction.metaphor}`, `- narrative: ${direction.visualNarrative}`, `- composition: ${direction.composition.layout}`, `- typography: ${direction.typographyCharacter}`, `- materials: ${direction.materialLanguage.join(', ')}`, `- interaction: ${direction.interactionConcept}`, `- domain-elements: ${direction.domainElements.join(', ')}`, `- avoid: ${direction.forbiddenPatterns.join('; ')}`].join('\n')).join('\n\n');
  return ['---', `project: ${brief.id}`, `count: ${directions.length}`, '---', '# Creative directions', 'Each direction is a different metaphor, material, composition, and interaction concept. Mode changes are not counted as direction changes.', rows, '', '<!-- lde-directions-data', JSON.stringify({ interpretation, directions }, null, 2), '-->', ''].join('\n');
}

function parseDirections(markdown: string): { interpretation: unknown; directions: CreativeDirection[] } {
...
async function requireDirections(cwd: string): Promise<{ interpretation: unknown; directions: CreativeDirection[] }> {
...
function selectedDirection(directions: CreativeDirection[]): CreativeDirection {
  const selected = directions.find((direction) => direction.status === 'approved' || direction.status === 'selected');
...
function brandMarkdown(brand: Brandkit): string {
  return ['---', `project: ${brand.projectId}`, `generated-at: ${brand.generatedAt}`, '---', '# Brand narrative', brand.narrative, '', '# Personality', ...brand.personality.map((item) => `- ${item}`), '', '# Tone of voice', ...brand.toneOfVoice.map((item) => `- ${item}`), '', '# Naming rationale', brand.namingRationale, '', '# Logo direction', brand.logoDirection, '', '# Wordmark direction', brand.wordmarkDirection, '', '# Symbol direction', brand.symbolDirection, '', '# Color system', ...brand.colors.map((color) => `- ${color.name}: ${color.hex} — ${color.role}`), '', '# Typography roles', ...brand.typography.map((font) => `- ${font.role}: ${font.family} — ${font.usage}`), '', '# Spacing and shape', ...brand.spacingPrinciples.map((item) => `- ${item}`), ...brand.shapeLanguage.map((item) => `- ${item}`), '', '# Iconography and illustration', brand.iconographyDirection, brand.illustrationDirection, '', '# Photography and motion', brand.photographyDirection, ...brand.motionPrinciples.map((item) => `- ${item}`), '', '# Favicon and social preview', brand.faviconConcept, brand.socialPreviewConcept, '', '# Usage and misuse', ...brand.usageRules.map((item) => `- Use: ${item}`), ...brand.misuseRules.map((item) => `- Avoid: ${item}`), '', '# Image-generation prompts', ...brand.imagePrompts.map((asset) => `- ${asset.role}: ${asset.prompt} (negative: ${asset.negativeConstraints.join(', ')})`), ''].join('\n');
}
...
export async function runCommand(argv: string[], context: CommandContext): Promise<void> {
  ...
  if (command === 'brief') {
    ...
    const briefInput: BriefInput = { name: flags.name ?? previous?.name ?? 'Untitled project', summary: flags.summary ?? previous?.summary ?? 'Describe the product in one concrete sentence.', domain: flags.domain ?? previous?.domain ?? 'unclassified product', tension: flags.tension ?? previous?.tension ?? '' };
    if (previous) {
    ...
    await writeFile(join(root, 'manifest.json'), JSON.stringify({ version: 1, projectId: brief.id, artifacts: ['BRIEF.md'] }, null, 2));
  ...
  if (command === 'directions') {
    ...
    await writeFile(join(root, 'DIRECTIONS.md'), directionsMarkdown(brief, interpretation, directions));
  ...
  if (command === 'refine') {
    ...
    const feedback = flags.feedback ?? argv.slice(1).filter((item) => !item.startsWith('--')).join(' ').trim();
  ...
  if (command === 'preview') {
    ...
    if (flags.serve) {
      ...
      serve({ fetch: createPreviewApp(html).fetch, port }, (info) => context.output.push(`Preview server listening on http://localhost:${info.port}`));
    }
    return;
  }
  if (command === 'brandkit') {
    ...
    const generated = await Promise.all(brand.imagePrompts.map((request) => imageProvider.generate(request)));
    ...
    for (const asset of generated) await writeFile(join(root, asset.file.replace(/^assets\//, 'assets/')), asset.content ?? '');
  ...
  if (command === 'export') {
    const files = await Promise.all(['BRIEF.md', 'DIRECTIONS.md', 'pages/landing.design.md', 'BRAND.md'].map(async (file) => ({ file, content: await readText(join(root, file)) })));
    const design = await requireDesign(context.cwd);
    const markdown = ['# LLM Design Engine export', `Project: ${design.id}`, '', '## Agent implementation instructions', ...design.agentInstructions.map((item) => `- ${item}`), '', '## Approved design document', files.find((item) => item.file === 'pages/landing.design.md')?.content ?? '', '', '## Source artifacts', ...files.filter((item) => item.file !== 'pages/landing.design.md').map((item) => `### ${item.file}\n${item.content}`), ''].join('\n');
    await writeFile(join(root, 'EXPORT.md'), markdown);
    context.output.push(`Exported agent package to ${join(root, 'EXPORT.md')}`);
    return;
  }
  context.output.push('Commands: init, brief, directions, generate, preview, refine, approve, brandkit, lint, export');
}
...
export function createApiApp(cwd: string): Hono {
  const app = new Hono();
  app.get('/api/manifest', async (context) => context.json(JSON.parse(await readText(join(designRoot(cwd), 'manifest.json')))));
  app.get('/api/design', async (context) => context.json(await requireDesign(cwd)));
  app.get('/api/brandkit', async (context) => context.json(JSON.parse(await readText(join(designRoot(cwd), 'brandkit.json')))));
  return app;
}

export function createProgram(context: CommandContext): Command {
  const program = new Command().name('lde').description('Creative director and design compiler for coding agents.').version('0.1.0');
  const commands = ['init', 'brief', 'directions', 'generate', 'preview', 'refine', 'approve', 'brandkit', 'lint', 'export'];
  for (const name of commands) {
    const command = program.command(name);
    if (name === 'brief') command.option('--name <name>').option('--summary <summary>').option('--domain <domain>').option('--tension <tension>');
    ...
    command.action(async (...args: unknown[]) => {
      ...
      const flags: string[] = [];
      for (const [key, value] of Object.entries(commandOptions)) if (typeof value === 'string') flags.push(`--${key}`, value); else if (value === true) flags.push(`--${key}`);
